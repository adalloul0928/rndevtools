...
struct SystemImageComposer: ImageComposing {
  ...
  private func secureWorkspace(_ token: String) throws -> (inputs: URL, outputs: URL) {
    let base = FileManager.default.homeDirectoryForCurrentUser
      ...
      .appendingPathComponent("PUMPD Devtools", isDirectory: true)
  ...
  private func drawFramed(
    ...
    if layout.bezel == .pumpdGenericV1 {
    ...
    let inset: CGFloat =
      layout.bezel == .pumpdGenericV1 ? max(8, min(frame.width, frame.height) * 0.012) : 0
