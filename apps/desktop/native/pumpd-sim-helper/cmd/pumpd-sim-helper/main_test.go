...
func TestRunDeniesEveryDirectMutationBeforeSimulatorAccess(t *testing.T) {
	tests := map[string]string{
		...
		"apply_profile":   `{"simulatorId":"AAAAAAAA-BBBB-CCCC-DDDD-EEEEEEEEEEEE","profileId":"pumpd-development","checkpointToken":"prepared","confirmation":"APPLY_EXPERIMENTAL_PROFILE","acknowledgement":"EXPERIMENTAL"}`,
	...
	for operation, payload := range tests {
		t.Run(operation, func(t *testing.T) {
			...
			if exitCode := runWithIO([]string{"pumpd-sim-helper"}, strings.NewReader(input), &output, authorizer); exitCode != 1 {
...
func TestRunPreservesDirectReadOnlyHandshake(t *testing.T) {
	...
	var output bytes.Buffer
	if exitCode := runWithIO([]string{"pumpd-sim-helper"}, strings.NewReader(input), &output, authorizer); exitCode != 0 {
