...
`rndevtools-sim-helper` links a pinned, reviewed PUMPD fork of SimSlim's public Go
...
- License: MIT, reproduced in `apps/devtools-desktop/THIRD_PARTY_NOTICES.md`
- Clean-base manifest: `UPSTREAM_SOURCE_SHA256SUMS`
- PUMPD patch: `PUMPD_PATCHSET_pumpd.1.patch`
...
PUMPD patch set `pumpd.1` changes exactly three files:
...
module graph contains only PUMPD plus `github.com/mobai-app/simslim v0.8.0`.
...
inventory/cleanup, and clone repair. PUMPD adds a bounded one-request JSON
...
manifests validate the exact source compiled into PUMPD.
...
3. Copy the complete non-test root package and license, then rebase and review
   the narrow PUMPD patch set. Regenerate the deterministic patch artifact.
