...
func TestValidateMatchesServerCommandUnion(t *testing.T) {
	tests := map[protocol.Command]string{
		...
		protocol.CommandSlimming:   `{"kind":"slimming","operation":"verify","udids":["` + testUDID + `"],"profileId":"pumpd-development"}`,
	}
...
func TestParseMergesDistinctFlagsAndStdin(t *testing.T) {
	parsed, err := Parse(
		...
		strings.NewReader(`{"profileId":"pumpd-development"}`),
		true,
	)
	if err != nil {
		t.Fatal(err)
	...
	if command["kind"] != "slimming" || command["operation"] != "preview" || command["profileId"] != "pumpd-development" {
