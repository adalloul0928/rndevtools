...
func TestRunEmitsCompactSuccessAndRejectedResponses(t *testing.T) {
	for name, serverResponse := range map[string]func(string) string{
		"success": func(id string) string {
			return `{"protocol":"pumpd-devtools/1","id":"` + id + `","ok":true,"result":{"healthy":true}}`
		},
		"rejected": func(id string) string {
			return `{"protocol":"pumpd-devtools/1","id":"` + id + `","ok":false,"error":{"code":"approval_required","message":"Approve in the app.","retryable":false,"recovery":"Open PUMPD Devtools."}}`
		},
	} {
...
func appTestServer(t *testing.T, response func(string) string) string {
	...
	socketPath := filepath.Join(directory, "pumpd-devtools.sock")
