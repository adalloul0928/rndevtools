# `pumpd-devtools` provenance

`pumpd-devtools` is original first-party AVAD Technologies code written for the
PUMPD Devtools desktop app. No source, generated code, protocol definitions, or
...
and is intentionally fixed to `pumpd-devtools/1`. The Go module has no external
module dependency: `go list -m all` must return only
`github.com/avadtechnologies/pumpd-devtools-cli`. Its only incorporated
