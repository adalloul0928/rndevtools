...
describe('desktop URL boundaries', () => {
	...
	it('maps only private renderer assets below the generated root', () => {
		const root = '/Applications/PUMPD.app/Contents/Resources/app.asar/dist/renderer';
		expect(packagedRendererAssetPath(root, PACKAGED_RENDERER_URL)).toBe(
			`${root}/index.html`
		);
		expect(
			packagedRendererAssetPath(root, 'pumpd-devtools://app/assets/index.js?v=1')
		).toBe(`${root}/assets/index.js`);
		expect(
			packagedRendererAssetPath(root, 'pumpd-devtools://other/index.html')
		).toBeUndefined();
		expect(
			packagedRendererAssetPath(root, 'pumpd-devtools://app/%2e%2e/main/index.js')
		).toBeUndefined();
		expect(
	...
	it('uses a packaged CSP that cannot connect to arbitrary loopback services', () => {
		expect(PACKAGED_RENDERER_CONTENT_SECURITY_POLICY).toContain("connect-src 'self'");
